...
/* Tung - for COR/VOX */
struct vad_op {
    void (*on_vad_enc_state)(int is_active, void *user_data);
    void (*on_vad_dec_state)(int is_active, void *user_data);
};
...
static void on_vad_enc_state_default(int is_active, void *udata) {
    (void)udata;
    PJ_LOG(3, (__FILE__, "on_vad_enc_state_default: vad=%s", is_active?"active":"inactive"));
}
static void on_vad_dec_state_default(int is_active, void *udata) {
    (void)udata;
    PJ_LOG(3, (__FILE__, "on_vad_dec_state_default: vad=%s", is_active?"active":"inactive"));
}
/*
 * Print error.
 */
        ...
        pjmedia_frame *frame )
{
    ...
    if (frame->type == PJMEDIA_FRAME_TYPE_AUDIO)
        ...
        ts_len = PJMEDIA_PIA_SPF(&stream->port.info) / PJMEDIA_PIA_CCNT(&stream->port.info);
    else
        ts_len = 0;
     ...
     */
            ...
            (stream->codec_param.info.frm_ptime * stream->codec_param.info.channel_cnt *
             stream->codec_param.info.clock_rate/1000 < PJ_ARRAY_SIZE(zero_frame)) )
    {
    ...
    if (frame_out.size == 0) {
        if (stream->is_streaming) {
            ...
            // Tung - for COR/VOX
            stream->op.on_vad_enc_state(0, stream->vad_data);
        }
    ...
    if (stream->is_streaming == PJ_FALSE) {
        ...
        stream->op.on_vad_enc_state(1, stream->vad_data);
    }
...

{
    ...
    /* Tung - for COR/VOX */
    stream->op.on_vad_enc_state = &on_vad_enc_state_default;
    stream->op.on_vad_dec_state = &on_vad_dec_state_default;
    stream->vad_data = NULL;
...
/* Tung - for COR/VOX */
PJ_DEF(pj_status_t) 
pjmedia_stream_set_vad_options( pjmedia_stream *stream, 
                                void (*on_vad_enc_state)(int active, void *udata),
                                void (*on_vad_dec_state)(int active, void *udata), void *vad_data)
{
    stream->op.on_vad_enc_state = on_vad_enc_state;
    stream->op.on_vad_dec_state = on_vad_dec_state;
    stream->vad_data = vad_data;
    return PJ_SUCCESS;
}
...
{
    pjmedia_codec *codec = stream->codec;
    return pjmedia_codec_update_vad(codec, threshold);
}
