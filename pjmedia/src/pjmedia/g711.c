...
#endif
/* TUNG - for COR/VOX */
static pj_status_t g711_update_vad(pjmedia_codec *codec, int threshold);
...
static pj_status_t g711_update_vad(pjmedia_codec *codec, int threshold) {
    struct g711_private *priv = (struct g711_private*) codec->codec_data;
    pjmedia_silence_det *vad = priv->vad;
    PJ_LOG(3, (__FILE__, "update VAD threshold"));
    return pjmedia_silence_det_set_fixed(vad, threshold);
}
